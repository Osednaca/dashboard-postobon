...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-8">
        <div class="flex items-center gap-2 text-sm text-text-light mb-2">
            ...
            <span class="text-text">Editar Ubicación</span>
        </div>
        <h1 class="text-2xl font-bold text-text">Editar Ubicación</h1>
        <p class="text-sm text-text-light mt-1">Actualiza la información de {{ $location->name }}</p>
    </div>
        ...
        <form action="{{ route('locations.update', $location) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="contact_name" name="contact_name" value="{{ old('contact_name', $location->contact_name) }}"
                           class="w-full px-4 py-2.5 rounded-lg border border-border text-sm text-text placeholder:text-text-muted focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" id="phone" name="phone" value="{{ old('phone', $location->phone) }}"
                           class="w-full px-4 py-2.5 rounded-lg border border-border text-sm text-text placeholder:text-text-muted focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                </div>
            </div>
