<x-app-layout>
    ...
    <div class="mx-auto max-w-3xl space-y-6">
        ...
        <form action="{{ route('schedules.update', $schedule) }}" method="POST" class="overflow-hidden rounded-2xl border border-border bg-white shadow-[0_1px_3px_rgba(0,0,0,0.08),0_12px_32px_rgba(45,87,149,0.06)]">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center justify-end gap-3 border-t border-border bg-surface px-6 py-4 sm:px-8">
                ...
                <button type="submit" class="rounded-lg bg-primary px-6 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-primary/90">Guardar cambios</button>
            </div>
        </form>
    </div>
</x-app-layout>
