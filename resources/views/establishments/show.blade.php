        ...
        <aside class="space-y-6">
            ...
            <section class="rounded-xl border border-border bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,0.04)] sm:p-6" x-data="{ revealPassword: false }">
                ...
                <dl class="mt-4 divide-y divide-border">
                    ...
                    <div class="grid gap-2 py-3 last:pb-0 sm:grid-cols-[8rem_1fr]"><dt class="text-sm text-text-muted">Password</dt><dd class="min-w-0">@if($wifiPasswordUnreadable)<div class="rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-sm leading-5 text-amber-900" role="alert"><p class="font-semibold">No se puede descifrar esta credencial.</p><p class="mt-1 text-xs">Probablemente fue guardada con otro <code>APP_KEY</code>. Reemplázala desde <a href="{{ route('establishments.edit', $establishment) }}" class="font-semibold text-primary hover:underline">Editar establecimiento</a>.</p></div>@elseif($wifiPassword)<div class="flex items-center gap-2"><span class="min-w-0 flex-1 break-all font-mono text-sm text-text" x-text="revealPassword ? {{ Js::from($wifiPassword) }} : '••••••••••••'"></span><button type="button" @click="revealPassword = !revealPassword" class="shrink-0 rounded-lg border border-border px-2.5 py-1.5 text-xs font-semibold text-text-light transition hover:bg-surface" x-text="revealPassword ? 'Ocultar' : 'Mostrar'" :aria-label="revealPassword ? 'Ocultar password Wi‑Fi' : 'Mostrar password Wi‑Fi'"></button></div>@else<span class="text-sm text-text">Sin configurar</span>@endif</dd></div>
                </dl>
            </section>
        </aside>
    </div>
