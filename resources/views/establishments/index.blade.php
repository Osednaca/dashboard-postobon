                    ...
                    <tbody class="divide-y divide-border">
                        @foreach($establishments as $establishment)
                            ...
                            <tr x-show="!search || {{ Js::from($searchable) }}.includes(search.toLowerCase())" class="transition hover:bg-surface/50">
                                <td class="px-5 py-4"><a href="{{ route('establishments.show', $establishment) }}" class="max-w-56 break-words font-semibold text-text transition hover:text-primary hover:underline">{{ $establishment->name }}</a><div class="mt-1 text-xs text-text-muted">{{ $establishment->businessType?->name ?? 'Sin tipo' }}</div></td>
