...
@section('content')
    <div class="text-center mb-6">
        <h1 class="text-2xl font-bold text-text">Restablecer contraseña</h1>
        <p class="mt-1 text-sm text-text-muted">Crea una nueva contraseña para tu cuenta</p>
    </div>
    ...
    <form method="POST" action="{{ route('password.update') }}" class="space-y-5">
        ...
        <div>
            ...
            <input type="email" name="email" id="email" value="{{ old('email', $request->email) }}" required readonly
                class="w-full px-3.5 py-2.5 rounded-lg border border-border bg-surface-dark text-sm text-text-muted cursor-not-allowed">
        </div>
