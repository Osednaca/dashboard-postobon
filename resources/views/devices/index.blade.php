...
<div x-data="{ 
    ...
    <!-- Bulk Format SD Confirmation Modal -->
    <div x-show="showBulkFormatSdModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="showBulkFormatSdModal = false"></div>
        <div class="relative bg-white rounded-xl shadow-xl border border-border p-6 w-full max-w-md" @click.away="showBulkFormatSdModal = false">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-10 h-10 rounded-full bg-danger/10 flex items-center justify-center">
                    <svg class="w-5 h-5 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                ...
                <div>
                    <h3 class="text-lg font-semibold text-text">¿Formatear SD en lote?</h3>
                </div>
            </div>
            <p class="text-sm text-text-light mb-2">Se formatearán las tarjetas SD de <strong class="text-text" x-text="selectedIds.length"></strong> dispositivos seleccionados.</p>
            <p class="text-xs text-text-muted mb-6"><strong>Nota:</strong> Se eliminarán todos los videos almacenados de dichos dispositivos. Los dispositivos permanecerán vinculados a tu cuenta Z2 Cloud.</p>
            <div class="flex justify-end gap-3">
                <button @click="showBulkFormatSdModal = false" class="px-4 py-2.5 rounded-lg border border-border text-sm font-medium text-text hover:bg-surface transition-colors">Cancelar</button>
                <form action="{{ route('devices.bulk-format-sd') }}" method="POST" class="inline">
                    @csrf
                    <template x-for="id in selectedIds" :key="id">
                        <input type="hidden" name="device_ids[]" :value="id">
                    </template>
